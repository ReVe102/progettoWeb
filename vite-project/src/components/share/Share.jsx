...
export default function Share() {
  ...
  const uploadImage = async () => {
    try {
      const response = await axios.post(`${backendUrl}/posts/uploadImage`, {
        base64: image
      }, {
        headers: {
  ...
  const getImage = async () => {
    try {
      const response = await axios.get(`${backendUrl}/posts/images`);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/posts/create`, newPost, {
        headers: {
          "Content-Type": "application/json"
        }
      });
